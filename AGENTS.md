...
**`ShellEnvironment`** — Resolves user's login shell PATH via `/bin/zsh -l -i -c env` to locate CLI binaries. Caches results. Removes `Codex`/`CLAUDE_CODE_ENTRYPOINT` from spawned process environment to prevent nested session detection.
